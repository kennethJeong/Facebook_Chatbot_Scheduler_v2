...
function WrongAccessQR()
{
	$send['text'] = "🎩: 올바른 접근이 아니에요.";
...
{
	$send['text'] = "이전 단계로 돌아가려면 아래 버튼을 눌러주세요";
	$send['title'] = array('이전으로', '초기화면', '등록된 과목 정보 보기');
	$send['payload'] = array('rgstRETURN', 'rgstRESET', 'readINFO');
	return messageQR($send);
}*/
/*
function returnRead()
{
	$send['text'] = "이전 단계로 돌아가려면 아래 버튼을 눌러주세요";
	$send['title'] = array('이전으로', '초기화면');
	$send['payload'] = array('readRETURN', 'readRESET');
	return messageQR($send);
}*/

function ReturningError()
{
	global $inProgress;
