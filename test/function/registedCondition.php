...
function registedConditionSubject($userInfo)
{
	for($i=0; $i<count($userInfo); $i++) {
		if($userInfo[$i]['title']) {
			if(!empty($userInfo[$i]['day2']) && !empty($userInfo[$i]['time2'])) {
				if($userInfo[$i]['time1'] == $userInfo[$i]['time2']) {
					$rgstedInfo[] = $userInfo[$i]['title'] . "(" . $userInfo[$i]['day1'] . ", " . $userInfo[$i]['day2'] . " - " . $userInfo[$i]['time1'] . ")";
				} else {
					$rgstedInfo[] = $userInfo[$i]['title'] . "(" . $userInfo[$i]['day1'] . " - " . $userInfo[$i]['time1'] . ", " . $userInfo[$i]['day2'] . " - " . $userInfo[$i]['time2'] . ")";
				}
			} else {
				$rgstedInfo[] = $userInfo[$i]['title'] . "(" . $userInfo[$i]['day1'] . " - " . $userInfo[$i]['time1'] . ")";
			}
		}
	}
...
function registedConditionSubjectDetail($userInfo)
{
	for($i=0; $i<count($userInfo); $i++) {
		if($userInfo[$i]['divs'] == "교양") {
			if(!empty($userInfo[$i]['time2']) || $userInfo[$i]['time2'] != "") {
				$rgstedInfoDetail[] = "교과목명: " . $userInfo[$i]['title'] . "\n\n과목 구분: " . $userInfo[$i]['divs'] . " - " . $userInfo[$i]['fields'] . " " . $userInfo[$i]['department'] . "\n\n분반 및 교수명: " . $userInfo[$i]['class'] . "분반, " . $userInfo[$i]['prof'] . " 교수님\n\n수업시간 1: " . $userInfo[$i]['day1'] . ", " . $userInfo[$i]['time1'] . "(" . $userInfo[$i]['min1'] . ")\n수업시간 2: " . $userInfo[$i]['day2'] . ", " . $userInfo[$i]['time2'] . "(" . $userInfo[$i]['min2'] . ")";
			} else {
				$rgstedInfoDetail[] = "교과목명: " . $userInfo[$i]['title'] . "\n\n과목 구분: " . $userInfo[$i]['divs'] . " - " . $userInfo[$i]['fields'] . " " . $userInfo[$i]['department'] . "\n\n분반 및 교수명: " . $userInfo[$i]['class'] . "분반, " . $userInfo[$i]['prof'] . " 교수님\n\n수업시간:  " . $userInfo[$i]['day1'] . ", " . $userInfo[$i]['time1'] . "(" . $userInfo[$i]['min1'] . ")";
			}
		}
		else if($userInfo[$i]['divs'] != "교양") {
			if(!empty($userInfo[$i]['time2']) || $userInfo[$i]['time2'] != "") {
				$rgstedInfoDetail[] = "교과목명: " . $userInfo[$i]['title'] . "\n\n과목 구분: " . $userInfo[$i]['divs'] . "\n\n수강학과: " . $userInfo[$i]['major'] . "\n\n분반 및 교수명: " . $userInfo[$i]['class'] . "분반, " . $userInfo[$i]['prof'] . " 교수님\n\n수업시간 1: " . $userInfo[$i]['day1'] . ", " . $userInfo[$i]['time1'] . "(" . $userInfo[$i]['min1'] . ")\n수업시간 2: " . $userInfo[$i]['day2'] . ", " . $userInfo[$i]['time2'] . "(" . $userInfo[$i]['min2'] . ")";
			} else {
				$rgstedInfoDetail[] = "교과목명: " . $userInfo[$i]['title'] . "\n\n과목 구분: " . $userInfo[$i]['divs'] . "\n\n수강학과: " . $userInfo[$i]['major'] . "\n\n분반 및 교수명: " . $userInfo[$i]['class'] . "분반, " . $userInfo[$i]['prof'] . " 교수님\n\n수업시간: " . $userInfo[$i]['day1'] . ", " . $userInfo[$i]['time1'] . "(" . $userInfo[$i]['min1'] . ")";			
			}
		}
...
function registedConditionSubjectDetail($userInfo)
{
	for($i=0; $i<count($userInfo); $i++) {
		...
		if($userInfo[$i]['classroom1'] == $userInfo[$i]['classroom2'] || (empty($userInfo[$i]['classroom2']) && $userInfo[$i]['classroom1'] != $userInfo[$i]['classroom2'])) {
			$classroom = "강의실: " .$userInfo[$i]['classroom1'];
		} else {
			$classroom = "강의실1: " . $userInfo[$i]['classroom1'] . "\n강의실2: " . $userInfo[$i]['classroom2'];
		}
		...
		if($userInfo[$i]['divs'] == "교양") {
			...
			$result['title'][] = "<" . $userInfo[$i]['title'] . "> " . $userInfo[$i]['divs'] . " - " . $userInfo[$i]['major'];
			if(!empty($userInfo[$i]['time2']) || $userInfo[$i]['time2'] != "") {
				$result['info'][] = "분반 및 교수명: " . $userInfo[$i]['class'] . "분반, " . $userInfo[$i]['prof'] . " 교수님\n" . $classroom . "\n수업시간 1: " . $userInfo[$i]['day1'] . ", " . $userInfo[$i]['time1'] . "(" . $userInfo[$i]['min1'] . ")\n수업시간 2: " . $userInfo[$i]['day2'] . ", " . $userInfo[$i]['time2'] . "(" . $userInfo[$i]['min2'] . ")";
			} else {
				$result['info'][] = "분반 및 교수명: " . $userInfo[$i]['class'] . "분반, " . $userInfo[$i]['prof'] . " 교수님\n" . $classroom . "\n수업시간: " . $userInfo[$i]['day1'] . ", " . $userInfo[$i]['time1'] . "(" . $userInfo[$i]['min1'] . ")";			
