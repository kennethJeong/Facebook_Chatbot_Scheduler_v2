...
{
	global $readTitle, $readContent, $inProgress;
...
{
	global $senderID, $conn;
	
	$query = "SELECT * FROM kennethj.noname_Event WHERE userkey='$senderID'";
	$sql4event = mysql_query($query, $conn);	
	while($row4event = mysql_fetch_assoc($sql4event)) {
		$eventInfo[] = $row4event;
	}
	
	for($i=0; $i<count($eventInfo); $i++) {
		if($eventInfo[$i]['type'] == "assignment") {
			for($j=0; $j<count($eventInfo[$i]['type']); $j++) {
				$deleteEvent['title'][] = "<과제 - " . $eventInfo[$i]['title'] . "> - " . $eventInfo[$i]['content'];
			...
			for($j=0; $j<count($eventInfo[$i]['type']); $j++) {
				if(empty($eventInfo[$i]['date2'])) {
					$deleteEvent['title'][] = "<휴강 - " . $eventInfo[$i]['title'] . "> - " . substr($eventInfo[$i]['date1'], 0, 2) . "월 " . substr($eventInfo[$i]['date1'], 2, 2) . "일";
				}
				else if(!empty($eventInfo[$i]['date2'])) {
					$deleteEvent['title'][] = "<휴강 - " . $eventInfo[$i]['title'] . "> - " . substr($readEent[$i]['date1'], 0, 2) . "월 " . substr($eventInfo[$i]['date1'], 2, 2) . "일부터 " . substr($eventInfo[$i]['date2'], 0, 2) . "월 " . substr($eventInfo[$i]['date2'], 2, 2) . "일까지";
				}
				$deleteEvent['payload'][] =  "readDelete_" . $eventInfo[$i]['type'] . "_" . $i;
			}
		}
			...
			for($j=0; $j<count($eventInfo[$i]['type']); $j++) {
				$deleteEvent['title'][] = "<시험 - " . $eventInfo[$i]['title'] . "> - " . substr($eventInfo[$i]['date1'], 0, 2) . "월 " . substr($eventInfo[$i]['date1'], 2, 2) . "일 / " . substr($eventInfo[$i]['time1'], 0, 2) . "시 " . substr($eventInfo[$i]['time1'], 2, 2) . "분";
				$deleteEvent['payload'][] =  "readDelete_" . $eventInfo[$i]['type'] . "_" . $i;
			}
		}
	}
